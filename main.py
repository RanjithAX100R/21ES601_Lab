...
SCREEN = pygame.display.set_mode((500, 550))  # Setting the display

# background
BACKGROUND_IMAGE = pygame.image.load('background.jpg')

#  bird
BIRD_PHOTO = pygame.image.load('bird.png')
bird_x = 50
bird_y = 100
bird_y_change = 0

def display_bird(x, y):
    SCREEN.blit(BIRD_PHOTO, (x, y))

# OBSTACLES
RECTANGLE_OBSTACLE_WIDTH = 50
OBSTACLE_HEIGHT = random.randint(150,450)
OBSTACLE_COLOR = (211, 253, 117)
OBSTACE_X_CHANGE = -0.1
RECTANGLE_OBSTACLE_X = 500

def display_obstacle(height):
    pygame.draw.rect(SCREEN, OBSTACLE_COLOR, (RECTANGLE_OBSTACLE_X, 0, RECTANGLE_OBSTACLE_WIDTH, height))
    bottom_obstacle_height = 635 - height - 150
    pygame.draw.rect(SCREEN, OBSTACLE_COLOR, (RECTANGLE_OBSTACLE_X, 35, RECTANGLE_OBSTACLE_WIDTH, -bottom_obstacle_height))

# COLLISION DETECTION
def collision_detection (RECTANGLE_OBSTACLE_X, obstacle_height, bird_y, bottom_obstacle_height):
    if RECTANGLE_OBSTACLE_X >= 50 and RECTANGLE_OBSTACLE_X <= (50 + 64):
...
while running:
    ...
    # Moving the obstacle
    RECTANGLE_OBSTACLE_X += OBSTACE_X_CHANGE

    # COLLISION
    collision = collision_detection(RECTANGLE_OBSTACLE_X, OBSTACLE_HEIGHT, bird_y, OBSTACLE_HEIGHT + 150)
